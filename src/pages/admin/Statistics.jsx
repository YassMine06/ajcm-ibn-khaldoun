...
export default function Statistics() {
  ...
  const [stats, setStats] = useState({ events: 0, annonces: 0, members: 0, registrations: 0 });
  const [recentEvents, setRecentEvents] = useState([]);

  const { execute: fetchStats, isLoading } = useAsync(async () => {
    const [ev, an, us, re] = await Promise.all([
      eventService.getAll(),
