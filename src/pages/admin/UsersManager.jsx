...
export default function UsersManager() {
  ...
  const { execute: fetchUsers, isLoading, error } = useAsync(userService.getAll);
  ...
  const handleSave = async (e) => {
    ...
    try {
      ...
      if (_isNew) {
        await userService.create(memberToSave);
      } else {
        await userService.update(currentMember.id, memberToSave);
      }
      setIsEditing(false);
      loadData();
    } catch (err) {
  ...
  const startAdd = () => {
    const nextNum = members.length > 0 ? Math.max(...members.map(m => {
      if (!m.id) return 0;
          ...
          <div className="data-table-container" style={{ padding: '2rem' }}>
            <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '1.5rem' }}>
              <div style={{ gridColumn: '1/-1', backgroundColor: '#EEF2FF', padding: '1rem', borderRadius: '10px', border: '1px solid #C7D2FE' }}>
                ...
                <input type="text" className="search-input" disabled={isSaving} placeholder="Ex: M-2026-001" style={{ fontWeight: 800, letterSpacing: '0.05em' }} value={currentMember.id || ''} onChange={e => setCurrentMember({...currentMember, id: e.target.value})} />
              ...
              <div>
                ...
                <input required type="text" className="search-input" disabled={isSaving} value={currentMember.name} onChange={e => setCurrentMember({...currentMember, name: e.target.value})} />
              </div>
              <div>
                <label style={{ display: 'block', fontWeight: 600, marginBottom: '0.5rem' }}>Date d'adhésion</label>
                <input required type="date" className="search-input" disabled={isSaving} value={currentMember.joinDate} onChange={e => setCurrentMember({...currentMember, joinDate: e.target.value})} />
              </div>
              <div>
                <label style={{ display: 'block', fontWeight: 600, marginBottom: '0.5rem' }}>Date de naissance</label>
                <input required type="date" className="search-input" disabled={isSaving} value={currentMember.birthDate} onChange={e => setCurrentMember({...currentMember, birthDate: e.target.value})} />
              ...
              <div>
                ...
                <input required type="password" placeholder="••••••••" disabled={isSaving} className="search-input" value={currentMember.password || ''} onChange={e => setCurrentMember({...currentMember, password: e.target.value})} />
              </div>
                ...
                <tr key={m.id}>
                  ...
                  <td>
                    <div style={{ fontSize: '0.85rem', color: 'var(--saas-text-muted)' }}>
                      ...
                      <div style={{ display: 'flex', alignItems: 'center', gap: '4px' }}><Calendar size={12} /> Adhésion: {m.joinDate ? new Date(m.joinDate).toLocaleDateString('fr-FR') : '--'}</div>
